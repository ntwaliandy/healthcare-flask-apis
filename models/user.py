...
class User:
    ...
    def UserRegister():
        try:
            ...
            user_dict = {"username": _username, "email": _email, "role": _role, "password": hashed_password, "created_at": _dateTime}
            add_data = DB().insert('healthcare.user', **user_dict)

            print(":: insert data ::", add_data)

            # making response
            response = make_response(100, user_dict)
            return response

        except Exception as e:
            print(e)
            response = make_response(403, "failed to register user")
            return response
    ...
    def getClientUsers():
        try:
            ...
            data = DB().select(sql)
            if len(data) <= 0:
                response = make_response(403, "No registered client yet")
            response = make_response(100, data)
            return response

        except Exception as e:
            print(e)
            response = make_response(403, "failed to pull all users")
            return response
    ...
    def userLogin():
        try:
            ...
            response = make_response(100, check_user[0])
            return response

        except Exception as e:
            print(e)
            response = make_response(403, "failed to logIn user")
            return response
    ...
    def updatePass():
        try:
            ...
            if (_oldPass == _newPass):
                response = make_response(403, "new password is similar to the old one")
                return response

            # checking new password length
            if len(_newPass) <= 5:
                response = make_response(403, "new password is too short")
                return response
    ...
    def getUserbyID():
        try:
            ...
            response = make_response(100, check_userExist[0])
            return response

        except Exception as e:
            print(e)
            response = make_response(403, "failed to retrieve user")
            return response
